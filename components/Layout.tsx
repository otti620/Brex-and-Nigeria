...
const Layout: React.FC<LayoutProps> = ({ children, activeScreen, onNavigate, hideNav = false, isAdmin = false }) => {
  const navItems = [
    { 
      id: Screen.Dashboard, 
      icon: (active: boolean) => (
        <svg className={`w-6 h-6 transition-colors duration-200 ${active ? 'text-[#ff9c00]' : 'text-[#64748B]'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <span className={`text-[10px] font-bold tracking-tight transition-colors duration-200 ${isActive ? 'text-[#ff9c00]' : 'text-[#64748B]'}`}>
                    {item.label}
                  </span>
                </button>
              );
